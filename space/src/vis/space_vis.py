...
class SpaceVis:
    ...
    def show_vis(self, model, dataset, indices, path, device):
        ...
        data = data.to(device)
        loss, log = model(torch.unsqueeze(data[1],0), 100000000)
        for key, value in log.items():
            if isinstance(value, torch.Tensor):
                log[key] = value.detach().cpu()
        log = AttrDict(log)
        ...
        log.z_pres = log.z_pres.squeeze(2) > 0.5
        print(log.z_pres)
        boxes = np.array(convert_to_boxes(log.z_where,log.z_pres, log.z_pres_prob.squeeze(2))).squeeze(0)
        print(boxes)
        for i in range(np.shape(boxes)[0]):
            img = cv2.imread("D:/Study/RL/random-network-distillation-pytorch-master/SPACE/data/ATARI/MontezumaRevenge-v0/validation/00000.jpg")
            print(img.shape)
            y_min, y_max, x_min, x_max = boxes[i][0],boxes[i][1],boxes[i][2],boxes[i][3]
            y_min *=128
            y_max *=128
            x_min *=128
            x_max *=128
            ...
            try:
                cv2.imwrite("D:/Study/RL/random-network-distillation-pytorch-master/SPACE/resultbbox/{}.jpg".format(i),roi)
            except:
                continue
        # print (fg_box)
        # (B, 1, 3, H, W)
